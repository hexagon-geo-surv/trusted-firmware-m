 ...
 *
 * Reminder: update profiles in x509_crt.c when adding a new curve!
 */
...
static int ecp_modp( mbedtls_mpi *N, const mbedtls_ecp_group *grp )
{
    ...
    /* N->MBEDTLS_PRIVATE(s) < 0 is a much faster test, which fails only if N is 0 */
 ...
 * N->MBEDTLS_PRIVATE(s) < 0 is a very fast test, which fails only if N is 0
 */
#define MOD_SUB( N )                                                    \
    while( (N).MBEDTLS_PRIVATE(s) < 0 && mbedtls_mpi_cmp_int( &(N), 0 ) != 0 )           \
        MBEDTLS_MPI_CHK( mbedtls_mpi_add_mpi( &(N), &(N), &grp->P ) )
 ...
 *
 * We accept Q->MBEDTLS_PRIVATE(Z) being unset (saving memory in tables) as meaning 1.
 *
 * Cost: 1A := 8M + 3S
 */
...
static int ecp_normalize_mxz( const mbedtls_ecp_group *grp, mbedtls_ecp_point *P )
{
    ...
    MBEDTLS_MPI_CHK( mbedtls_mpi_mul_mod( grp, &P->MBEDTLS_PRIVATE(X), &P->MBEDTLS_PRIVATE(X), &P->MBEDTLS_PRIVATE(Z) ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_lset( &P->MBEDTLS_PRIVATE(Z), 1 ) );
                ...
                int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    MBEDTLS_MPI_CHK( mbedtls_mpi_mul_mod( grp, &P->MBEDTLS_PRIVATE(Z), &P->MBEDTLS_PRIVATE(Z), &l ) );
                               ...
                               const mbedtls_mpi *d )
{
    ...
    MBEDTLS_MPI_CHK( mbedtls_mpi_add_mod( grp, &A,    &P->MBEDTLS_PRIVATE(X),   &P->MBEDTLS_PRIVATE(Z) ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_mul_mod( grp, &AA,   &A,      &A    ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_sub_mod( grp, &B,    &P->MBEDTLS_PRIVATE(X),   &P->MBEDTLS_PRIVATE(Z) ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_mul_mod( grp, &BB,   &B,      &B    ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_sub_mod( grp, &E,    &AA,     &BB   ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_add_mod( grp, &C,    &Q->MBEDTLS_PRIVATE(X),   &Q->MBEDTLS_PRIVATE(Z) ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_sub_mod( grp, &D,    &Q->MBEDTLS_PRIVATE(X),   &Q->MBEDTLS_PRIVATE(Z) ) );
    ...
    MBEDTLS_MPI_CHK( mbedtls_mpi_sub_mod( grp, &S->MBEDTLS_PRIVATE(Z), &DA,     &CB   ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_mul_mod( grp, &S->MBEDTLS_PRIVATE(Z), &S->MBEDTLS_PRIVATE(Z),   &S->MBEDTLS_PRIVATE(Z) ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_mul_mod( grp, &S->MBEDTLS_PRIVATE(Z), d,       &S->MBEDTLS_PRIVATE(Z) ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_mul_mod( grp, &R->MBEDTLS_PRIVATE(X), &AA,     &BB   ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_mul_mod( grp, &R->MBEDTLS_PRIVATE(Z), &grp->A, &E    ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_add_mod( grp, &R->MBEDTLS_PRIVATE(Z), &BB,     &R->MBEDTLS_PRIVATE(Z) ) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_mul_mod( grp, &R->MBEDTLS_PRIVATE(Z), &E,      &R->MBEDTLS_PRIVATE(Z) ) );
